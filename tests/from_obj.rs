...
fn test_from_obj_two_pawns() -> Result<()> {
    ...
    assert_eq!(
        n,
        366,
        "expected 366 merged vertices, got {}",
        pawns.points.len()
    );
    assert_eq!(
        pawns.index_triangles.len(),
        724,
        "expected 724 merged triangles, got {}",
        pawns.index_triangles.len()
    );
    ...
    for (idx, triangle) in pawns.index_triangles.iter().enumerate() {
        assert!(
            triangle.i < n,
            "triangle {idx}: i={} out of bounds (n={n})",
            triangle.i
        );
        assert!(
            triangle.j < n,
            "triangle {idx}: j={} out of bounds (n={n})",
            triangle.j
        );
        assert!(
            triangle.k < n,
            "triangle {idx}: k={} out of bounds (n={n})",
            triangle.k
        );
    }
